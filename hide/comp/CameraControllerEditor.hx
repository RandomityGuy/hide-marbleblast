...
class CameraControllerEditor extends Popup {
	...
	function refresh() {
		var ortho = Type.getClass(editor.cameraController) == OrthoController;

		var fly = Type.getClass(editor.cameraController) == FPSController || Type.getClass(editor.cameraController) == FlightController;
	...
	function create() {
...

		{
			...
			if (ortho != null)
				range.value = ortho.orthoZBounds;
			range.onChange = function(_) {
				var ortho = Std.downcast(editor.cameraController, OrthoController);
				if (ortho != null)
					ortho.orthoZBounds = range.value;
			};
		}

		{
			var dd = new Element('<br><input type="button" value="Preview Camera Path"/>').appendTo(form_div);
			dd.on("click", function(_) {
				cast(editor.view, hide.view.Prefab).doCameraPathAnimation();

				var stopAnimationFunc;
				stopAnimationFunc = (e) -> {
					cast(editor.view, hide.view.Prefab).stopCameraPathAnimation();
					js.Browser.window.removeEventListener('keydown', stopAnimationFunc);
				};

				js.Browser.window.addEventListener('keydown', stopAnimationFunc);
			});
		}
	}
}
