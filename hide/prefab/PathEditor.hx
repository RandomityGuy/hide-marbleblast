...
import hrt.prefab.l3d.InteriorPath.InteriorPathMarker;

#if editor
typedef MarkerPointData = {
	pos:h3d.Vector,
	prev:InteriorPathMarker,
	next:InteriorPathMarker,
}

class NewMarkerPointViewer extends h3d.scene.Object {
	var pointViewer:h3d.scene.Mesh;
	var connectionViewer:h3d.scene.Graphics;

	public function new(parent:h3d.scene.Object) {
		super(parent);
		name = "MarkerPointViewer";
	...
	public function update(spd:MarkerPointData) {
		...
		if (connectionViewer.visible) {
			var startPos = spd.prev == null ? spd.next.getAbsPos().getPosition() : spd.prev.getAbsPos().getPosition();
...
@:access(hrt.prefab.l3d.InteriorPath)
class PathEditor {
	public var prefab:hrt.prefab.l3d.InteriorPath;
	...
	var newSplinePointViewer:NewMarkerPointViewer;

	public function new(prefab:hrt.prefab.l3d.InteriorPath, undo:hide.ui.UndoHistory) {
		this.prefab = prefab;
		this.undo = undo;
	}
	...
	function getClosestSplinePointFromMouse(mouseX:Float, mouseY:Float, ctx:hrt.prefab.Context):InteriorPathMarker {
		...
		var minDist = -1.0;
		var result:InteriorPathMarker = null;
		for (sp in prefab.markers) {
			var screenPos = sp.getAbsPos().getPosition();
	...
	function getNewPointPosition(mouseX:Float, mouseY:Float, ctx:hrt.prefab.Context):MarkerPointData {
		if (prefab.markers.length == 0) {
			return {
				pos: ctx.local3d.getAbsPos().getPosition(),
				prev: null,
				next: null
			};
		}
		...
		if ((closestPt.next == null || closestPt.prev == null)) {
	...
	function getClosestPointFromMouse(mouseX:Float, mouseY:Float, ctx:hrt.prefab.Context):MarkerPointData {
		if (ctx == null || ctx.local3d == null || ctx.local3d.getScene() == null)
			return null;

		var result:MarkerPointData = null;
		var mousePos = new h3d.Vector(mouseX / h3d.Engine.getCurrent().width, 1.0 - mouseY / h3d.Engine.getCurrent().height, 0);
		var minDist = -1.0;
		for (i in 0...prefab.markers.length) {
			var s = prefab.markers[i];
			var screenPos = s.getAbsPos().getPosition();
			screenPos.project(ctx.local3d.getScene().camera.m);
			screenPos.z = 0;
			screenPos.scale3(0.5);
			screenPos = screenPos.add(new h3d.Vector(0.5, 0.5));
			...
			if ((dist < minDist || minDist == -1) && dist < 0.1) {
				minDist = dist;
				result = {
					pos: s.getAbsPos().getPosition(),
					prev: i == 0 ? null : prefab.markers[i - 1],
					next: i == prefab.markers.length - 1 ? null : prefab.markers[i + 1]
				}
			}
		}
		...
		if (result == null) {
			...
			var firstSp = prefab.markers[0];
			var firstPt = firstSp.getAbsPos().getPosition();
			var firstPtScreenPos = firstPt.clone();
			...
			var lastSp = prefab.markers[prefab.markers.length - 1];
			var lastPt = lastSp.getAbsPos().getPosition();
			var lastPtSreenPos = lastPt.clone();
			...
			if (distTolastPoint < distToFirstPoint) {
				result.pos = lastPt;
				result.prev = prefab.markers[prefab.markers.length - 1];
				result.next = null;
			} else {
				result.pos = firstPt;
				result.prev = null;
				result.next = prefab.markers[0];
			}
		}
	...
	function addSplinePoint(spd:MarkerPointData, ctx:hrt.prefab.Context):InteriorPathMarker {
		...
		if (spd.prev == null && spd.next == null) {
			...
			index = prefab.markers[0].seqNum;
			scale = prefab.markers[0].getAbsPos().getScale().x;
		} else if (spd.next == null) {
			index = prefab.markers[prefab.markers.length - 1].seqNum + 1;
			scale = prefab.markers[prefab.markers.length - 1].getAbsPos().getScale().x;
		} else {
			index = spd.next.seqNum;
			scale = (spd.prev.scaleX + spd.next.scaleX) * 0.5;
		}

		var sp = new InteriorPathMarker(prefab);
		sp.x = pos.x;
		sp.y = pos.y;
		sp.z = pos.z;
		prefab.children.remove(sp);
		prefab.children.push(sp);
		for (i in index...prefab.markers.length) {
			prefab.markers[i].seqNum++;
		}
		prefab.markers.insert(index, sp);
		sp.scaleX = scale;
		sp.scaleY = scale;
		sp.scaleZ = scale;
		sp.interiorPath = prefab;
		sp.seqNum = index;
		sp.name = "Marker";
		sp.msToNext = 5000;
		sp.smoothingType = spd.prev != null ? spd.prev.smoothingType : (spd.next != null ? spd.next.smoothingType : Linear);
		...
		for (sp in prefab.markers) {
			...
			gizmo.onStartMove = function(mode) {
				...
				gizmo.onMove = function(translate:h3d.Vector, rot:h3d.Quat, scale:h3d.Vector) {
					...
					if (sceneEditor.snapToGround
						&& (mode == MoveXY || mode == MoveX || mode == MoveY || mode == MoveZ || mode == MoveYZ || mode == MoveZX)) {
		...
		if (editMode) {
			...
			interactive.onPush = function(e) {
				...
				if (K.isDown(K.MOUSE_LEFT) && K.isDown(K.CTRL)) {
					...
					var index = prefab.markers.indexOf(sp);

					undo.change(Custom(function(undo) {
						if (undo) {
							editContext.scene.editor.deleteElements([sp], () -> {}, false, false);
							for (i in (index + 1)...prefab.markers.length) {
								prefab.markers[i].seqNum--;
							}
							prefab.markers.remove(sp);
							prefab.updateInstance(ctx);
							@:privateAccess editContext.scene.editor.refresh(Partial);
							prefab.updateInstance(ctx);
							createGizmos(ctx);
			...
			interactive.onMove = function(e) {
				...
				var index = prefab.markers.indexOf(closetSp);
				for (g in gizmos) {
				...
				if (K.isDown(K.CTRL)) {
					if (newSplinePointViewer == null)
						newSplinePointViewer = new NewMarkerPointViewer(ctx.local3d.getScene());
				...
				if (K.isDown(K.SHIFT)) {
					var index = prefab.markers.indexOf(getClosestSplinePointFromMouse(s2d.mouseX, s2d.mouseY, ctx));
					for (sp in prefab.markers) {
						if (index == prefab.markers.indexOf(sp))
	...
	public function edit(ctx:EditContext) {
		...
		reverseButton.click(function(_) {
			prefab.markers.reverse();
			for (i in 0...prefab.markers.length) {
				prefab.markers[i].seqNum = i;
			}
			@:privateAccess editContext.scene.editor.refresh(Partial);
