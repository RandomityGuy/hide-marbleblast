...
class MisParser {
	...
	public function parse() {
		...
		while (true) {
			...
			if (blockMatch && indexIsInStringLiteral(this.text, blockCommentRegEx.matchedPos().pos))
				blockMatch = false;
			if (lineMatch && indexIsInStringLiteral(this.text, lineCommentRegEx.matchedPos().pos))
				lineMatch = false;
	...
	function readElement() {
		...
		switch (type) {
			...
			default:
				...
				var endingBraceIndex = indexOfIgnoreStringLiterals(this.text, '};', this.index);
	...
	function hasNextElement() {
		if (!elementHeadRegEx.matchSub(this.text, Std.int(Math.min(this.text.length - 1, this.index))))
			return false;
		if (indexOfIgnoreStringLiterals(this.text.substring(this.index, elementHeadRegEx.matchedPos().pos), '}') != -1)
			return false;
		return true;
	}
	...
	function readValues() {
		...
		var statements = splitIgnoreStringLiterals(section, ';').map(x -> StringTools.trim(x)); // Get a list of all statements
		for (statement in statements) {
			...
			if (StringTools.endsWith(key, ']')) {
				...
				var index = Std.parseInt(key.substring(openingIndex + 1, closingIndex));
	...
	function copyFields(obj:MissionElementBase) {
			...
			if (value.length > 1) {
				for (i in 0...value.length) {
					...
					if (objfields.contains(fname))
						Reflect.setField(obj, fname, normalizeValue(value[i]));
					else
						obj._dynamicFields.set(fname, normalizeValue(value[i]));
				}
			} else {
				if (key == "static")
					key = "isStatic";
				if (objfields.contains(key))
					Reflect.setField(obj, key, normalizeValue(value[0]));
				else
					obj._dynamicFields.set(key, normalizeValue(value[0]));
			}
		}
	}
	...
	function resolveExpression(expr:String) {
		var parts = splitIgnoreStringLiterals(expr, '@').map(x -> {
			...
			if (StringTools.startsWith(x, '$') && this.variables[x] != null) {
				...
				x = unescape(x.substring(1, x.length - 1)); // It' s a string literal, so remove " "
			}
			return x;
		});
		return parts.join('');
