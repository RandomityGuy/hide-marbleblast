...
class HeightMapTile {
		...
		if (height == null) {
			for (t in hmap.textures)
				if (t.kind == Height && t.enable && t.path != null) {
					...
					if (path == t.path && (tx != 0 || ty != 0))
						continue;
					height = try hxd.res.Loader.currentInstance.load(path).toImage().getPixels() catch (e:hxd.res.NotFound) #if editor try
						hxd.res.Any.fromBytes(path, sys.io.File.getBytes(hide.Ide.inst.getPath(path))).toImage().getPixels()
					catch (e:Dynamic) #end
						null;
					break;
				}
			if (height == null)
				height = hxd.Pixels.alloc(1, 1, R32F);
			height.convert(R32F);
		}
		return height;
	}
	...
	public function create(mesh:HeightMapMesh) {
		...
		inline function getTextures(k)
			return hmap.getTextures(k, tx, ty);
		...
		if (htex != null)
			shader.heightOffset.set((hmap.heightFlipX ? -1 : 1) / htex.width, (hmap.heightFlipY ? -1 : 1) / htex.height);
		...
		shader.splats = splat;
		for (t in splat)
			t.filter = hmap.splatNearest ? Nearest : Linear;
...
class HeightMap extends Object3D {
	...
	override function load(obj:Dynamic) {
		super.load(obj);
		textures = [
			for (o in (obj.textures : Array<Dynamic>)) {
				path: o.path,
				kind: o.kind,
	...
	function getAlbedoProps() {
		...
		albedoProps = [
			for (t in textures)
				if (t.kind == Albedo) t.props == null ? new h3d.Vector(1, 1, 1, albedoTiling) : {
	...
	function getTile(x:Int, y:Int) {
		if ((sizeX > 0 && sizeY > 0 && (x < 0 || y < 0 || x >= sizeX || y >= sizeY))
			|| (sizeX == 0 && sizeY == 0 && (x != 0 || y != 0) && !autoSize)) {
			if (emptyTile == null)
	...
	function getTextureArray(k:HeightMaPTexturePathKind) {
		...
		if (tl.length == 0)
			tl = [h3d.mat.Texture.fromColor(k == Normal ? 0x8080FF : 0xFF00FF)];
		var indexes = [];
		var layers = [];
		for (t in tl) {
		...
		if (pl.length == 0)
			defaultTexture = h3d.mat.Texture.fromColor(k == Normal ? 0x8080FF : 0xFF00FF);
		var indexes = [];
		var layers = [];
		for (p in pl) {
...
class HeightGrid extends h3d.prim.Primitive {
	...
	**/
	...
	override function alloc(engine:h3d.Engine) {
		dispose();
		var format = if (hasNormals && hasUVs) hxd.BufferFormat.POS3D_NORMAL_UV; else if (hasNormals) hxd.BufferFormat.POS3D_NORMAL else if (hasUVs)
			hxd.BufferFormat.POS3D_UV else hxd.BufferFormat.POS3D;
