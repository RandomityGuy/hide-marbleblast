...
class MeshSpray extends Spray {
	...
	override function createObject(ctx:Context) {
		...
		inline function loadBatchMesh(source:String, mesh:h3d.scene.Mesh) {
			...
			if (multi != null)
				batch.materials = multi.materials;
			curMap.set(source, {
				batch: batch,
				pivot: mesh.defaultTransform.isIdentity() ? null : mesh.defaultTransform,
				emitCountTip: 1,
				init: false
			});
		...
		inline function loadBatch(source:String) {
			...
			if (obj.isMesh()) {
				...
				if (mesh != null)
					loadBatchMesh(source, mesh.toMesh());
			}
		}
		for (c in children) {
...
class MeshSprayObject extends Spray.SprayObject {
	...
	override public function redraw(updateShaders = false) {
		...
		for (c in children) {
			...
			if (m == null || !Std.isOfType(m.primitive, h3d.prim.MeshPrimitive))
				continue;
...
class MeshSpray extends Spray {
	...
	function saveToBinary() {
		...
		for (c in children.copy()) {
			if (c.type != "model" || c.children.length != 0 || !meshes.exists(c.source))
				continue;
			var c = c.to(Model);
			binaryMeshes.push({
				path: c.source,
	...
	override function edit(ectx:EditContext) {
		...
		if (sceneEditor == null) {
			allSetGroups = if (sys.FileSystem.exists(MESH_SPRAY_CONFIG_PATH)) try
				hide.Ide.inst.parseJSON(sys.io.File.getContent(MESH_SPRAY_CONFIG_PATH))
			catch (e:Dynamic)
				throw e + " (in " + MESH_SPRAY_CONFIG_PATH + ")"; else [];
		}
		...
		function createMeshElement(path:String) {
			...
			elt.contextmenu(function(e) {
				...
				new hide.comp.ContextMenu([
					{
						label: "Swap Model",
						click: function() hide.Ide.inst.chooseFile(["fbx", "l3d"], function(newPath) {
