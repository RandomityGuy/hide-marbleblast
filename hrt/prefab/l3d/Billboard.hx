...
class BillboardShader extends hxsl.Shader {
	static var SRC = {
		...
		function vertex() {
			var newModelView = mat4(vec4(camera.view[0].x, camera.view[1].x, camera.view[2].x, global.modelView[0].w),
				vec4(camera.view[0].y, camera.view[1].y, camera.view[2].y, global.modelView[1].w),
				vec4(camera.view[0].z, camera.view[1].z, camera.view[2].z, global.modelView[2].w), vec4(0, 0, 0, 1));

			// scale
			newModelView = mat4(vec4(scale /*length(global.modelView[0].xyz)*/, 0.0, 0.0, 0.0),
				vec4(0.0, scale /*length(global.modelView[1].xyz)*/, 0.0, 0.0), vec4(0.0, 0.0, scale /*length(global.modelView[2].xyz)*/, 0.0),
