...
class Layers2D extends hrt.prefab.Object3D {
	...
	function loadPixels(path:String) {
		return try hxd.res.Loader.currentInstance.load(path).toImage().getPixels() catch (e:hxd.res.NotFound) #if editor try
	...
	function saveUploadPixels() {
		...
		undo.change(Custom(function(undo) {
			var revertDataToApply = if (undo) revertList[--revertCurrentIdx]; else revertList[revertCurrentIdx++];
			...
			if (worldPos != null) {
				...
				if (clean) {
					...
					inline function isCollide(ix:Int, iy:Int) {
						return collideEnable
							&& collidePixels.getPixel(Math.floor(ix * collideScale), Math.floor(iy * collideScale)) & collideMask == collideMask;
					}
