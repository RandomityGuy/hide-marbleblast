...
class SplinePoint extends Object3D {
	...
	override function getHideProps():HideProps {
		return {
			icon: "arrows-v",
			name: "SplinePoint",
			allowParent: function(p) return p.to(Spline) != null,
...
class Spline extends Object3D {
	...
	inline function getMaxLengthBetween(p1:SplinePoint, p2:SplinePoint):Float {
		switch shape {
			...
			case Quadratic:
				return p1.getPoint().distance(p1.getSecondControlPoint()) + p1.getFirstControlPoint().distance(p2.getPoint());
			case Cubic:
				return p1.getPoint().distance(p1.getSecondControlPoint())
					+ p1.getFirstControlPoint().distance(p2.getFirstControlPoint())
					+ p2.getFirstControlPoint().distance(p2.getPoint());
		}
	}
	...
	function computeSplineData() {
		...
		// Sample the spline
		var samples:Array<SplinePointData> = [
			{
				pos: points[0].getPoint(),
				tangent: points[0].getTangent(),
				prev: points[0],
				next: points[1]
			}
		];
		...
		for (i in 1...maxI + 1) {
			...
			while (t <= 1.) {
				...
				if (p.distance(samples[samples.length - 1].pos) >= 1. / step)
					samples.insert(samples.length, {
						pos: p,
						tangent: getTangentBetween(t, curP, nextP),
						prev: curP,
						next: nextP
					});
				t += 1. / step;
